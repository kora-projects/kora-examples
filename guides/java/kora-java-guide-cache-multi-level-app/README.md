# Kora Guide Cache Multi-Level App

This module is the runnable Java/Gradle companion application for the [Cache Multi-Level guide](../../agents-md/kora-docs/mkdocs/docs/en/guides/cache-multi-level.md). It demonstrates a multi-level cache setup where local and remote cache layers cooperate behind Kora cache annotations.
...
## Run Checks
```bash
./gradlew :guides:java:kora-java-guide-cache-multi-level-app:classes
./gradlew :guides:java:kora-java-guide-cache-multi-level-app:test
```
...
```shell
./gradlew :guides:java:kora-java-guide-cache-multi-level-app:testClasses
```

Build the runnable distribution archive:

```shell
./gradlew :guides:java:kora-java-guide-cache-multi-level-app:distTar
```
...
```shell
./gradlew :guides:java:kora-java-guide-cache-multi-level-app:run
```
...
```shell
docker compose -f guides/java/kora-java-guide-cache-multi-level-app/docker-compose.yml up -d
```

Stop it when finished:

```shell
docker compose -f guides/java/kora-java-guide-cache-multi-level-app/docker-compose.yml down
```
