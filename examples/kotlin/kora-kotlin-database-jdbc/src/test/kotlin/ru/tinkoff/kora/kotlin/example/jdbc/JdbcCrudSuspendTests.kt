...
class JdbcCrudSuspendTests : KoraAppTestConfigModifier {
    @ConnectionPostgreSQL
    lateinit var connection: JdbcConnection

    @TestComponent
    lateinit var repository: JdbcCrudSuspendRepository

    override fun config(): KoraConfigModification = jdbcConfig(connection)

    @Test
    fun suspendSingleSuccess() = runBlocking {
        repository.insert(JdbcCrudSuspendRepository.Entity("1", 1, "2", null))
        assertEquals(1, repository.findById("1")?.field1)
        repository.update(JdbcCrudSuspendRepository.Entity("1", 5, "6", null))
