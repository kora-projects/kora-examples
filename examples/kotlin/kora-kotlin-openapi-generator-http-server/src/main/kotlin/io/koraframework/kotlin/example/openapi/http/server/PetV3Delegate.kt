...
import io.koraframework.kotlin.example.openapi.petV3.api.PetApiDelegate
import io.koraframework.kotlin.example.openapi.petV3.api.PetApiResponses
import io.koraframework.kotlin.example.openapi.petV3.model.Message
...
class PetV3Delegate : PetApiDelegate {
    ...
    override fun findPetsByStatus(status: String?): PetApiResponses.FindPetsByStatusApiResponse {
        val statusEnum = status?.let(::statusOf)
    ...
    override fun updatePetWithForm(
        ...
        val updated = pet.copy(
            name = name ?: pet.name,
            status = status?.let(::statusOf) ?: pet.status,
        )
