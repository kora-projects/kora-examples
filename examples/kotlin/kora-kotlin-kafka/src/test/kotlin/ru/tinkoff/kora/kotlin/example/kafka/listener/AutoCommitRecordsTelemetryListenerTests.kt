...
class AutoCommitRecordsTelemetryListenerTests : KoraAppTestConfigModifier {
    @ConnectionKafka
    ...
    @Test
    fun processed() {
        connection.send("my-topic-consumer", Event.ofValueAndRandomKey("""{"username":"Bob","code":1}"""))

        awaitReceived(consumer)
        assertEquals("""{"username":"Bob","code":1}""", consumer.received()[0])
    }
}
