...
| Дефектов фреймворка исправлено | 27 |
...
native-образ собран на GraalVM CE 25.2.4 (`native-image 25.0.4`) и проверен в работе: бинарь запущен против реальных
...
По ходу миграции найдено и исправлено **двадцать семь дефектов самого фреймворка** — каждый
отдельной веткой от `master`. Все двадцать семь отправлены в upstream как отдельные pull request'ы
[#791–#818](https://github.com/kora-projects/kora/pulls?q=is%3Apr+author%3Adsudomoin);
...
| все логи вне запроса теряются `KoraAsyncAppender`'ом | приложение работает, в логе просто нет части событий — заметно только когда по ним чего-то ждёшь |
| `ConfigWatcher` стартует до инициализации узла конфигурации | стектрейс при каждом старте и молча неработающее слежение за конфигом |
...
Три записи журнала дефектов остаются незакрытыми — все три требуют решения со стороны мейнтейнеров,
а не патча:

- **Диагностика `GraphBuilder` при провале всех форков шаблона.** Дефект подтверждён контролируемым
  экспериментом и описан, но не исправлен: нужен дизайн вывода (сводка по всем кандидатам вместо
  одного случайного), а не точечная правка.
- **KSP-процессоры падают при параллельном прогоне Gradle** (`JacksonIOException: Stream closed`).
  Воспроизводится стабильно, обходится `--max-workers=1`. Причина — общий разделяемый поток вывода
  между воркерами; исправление затрагивает контракт процессоров.
- **Генератор OpenAPI схлопывает две одинаковые именованные схемы в одну.** Записано как
  наблюдение, до корневой причины не доводилось: на примерах репозитория не блокирует.

`ConfigWatcher` из этого списка ушёл — исправлен в
[#818](https://github.com/kora-projects/kora/pull/818): узел конфигурации теперь приходит
в `ConfigWatcher` ещё и значением, поэтому граф обязан инициализировать его раньше и ожидание
с `catch (IllegalStateException)` стало не нужно.
...
Про колонку «Native» у `examples/graalvm/*`: образ собран `nativeCompile` на GraalVM CE 25.2.4
