...
| `KORA_2_FINAL_REPORT.md` | итоговый отчёт: сводка по всем модулям, правилам, дефектам и незакрытым вопросам |
| `KORA_2_PULL_REQUESTS.md` | разбор каждого из 27 отправленных PR во фреймворк |
...
| `openrewrite/` | AST-безопасные рецепты для Java и Gradle — состав, запуск и границы применимости в `openrewrite/README.md` |
...
```shell
# 1. Пакеты, координаты, версии, Kotlin, ресурсы, перенос каталогов пакетов
...
# 2. Первая сборка после смены пакетов — обязательно так
export JAVA_HOME=<JDK 25>
./gradlew clean --continue
./gradlew classes testClasses --continue --no-build-cache
```

Именно так мигрировался этот репозиторий. OpenRewrite-рецепт здесь не использовался: он не знает Kotlin,
а его рецепты по координатам требуют, чтобы новые координаты резолвились в момент применения
— того, что скрипт делает одним проходом вместе со сменой версии. Для чисто Java-проектов,
где текстовые замены нежелательны, рецепт применяется из самого мигрируемого проекта —
см. `openrewrite/README.md`.
